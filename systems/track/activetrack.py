...
class Segment:
    ...
    def cog_mean(self) -> Optional[float]:
        ...
        if not self.cog_samples:
            return None
        r = sum(math.radians(c) for c in self.cog_samples)
        i = sum(math.sin(r_) for r_ in [math.radians(c) for c in self.cog_samples])
        c = sum(math.cos(r_) for r_ in [math.radians(c) for c in self.cog_samples])
        if c == 0 and i == 0:
